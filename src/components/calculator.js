...
class Calculator extends React.Component {
  ...
  handleOperators(e) {
    if (!this.state.currentVal.includes("Limit")) {
      const value = this.parseOperators(e.currentTarget.getAttribute("id"));
    ...
    return (
      <Fragment>
        <div className="result" id="result">
          <small className="white">{formula}</small>
          <h2 className="title">{currentVal}</h2>
        </div>
        <section className="section layout">
          <div className="container">
            <nav className="panel">
              <p className="panel-heading is-active">Calculator</p>
              <InputBar formula={formula.replace(/x/g, "*")} />
              <Numberpad {...delegate} />
            </nav>
          </div>
        </section>
